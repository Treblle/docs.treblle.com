...
Custom Spectral rules integrate seamlessly with Treblle's existing governance framework:
...
4. **Unified Reporting**: Custom and default governance checks appear together on the API Governance page
...
- Custom rules supplement (not replace) Treblle's default governance checks
- You can change which ruleset is applied to an API at any time
