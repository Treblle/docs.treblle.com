...
- The [endpoint group](/treblle/endpoints#endpoint-groups) the endpoint belongs to
...
Treblle automatically groups your endpoints. For example if you have two endpoints `/users/:id` and `/users/verify`, Treblle will group then under the `users` endpoint group. This makes it easy to [filter](/treblle/endpoints#filters) for endpoints by groups.
...
- HTTP method
- [Endpoint group](/treblle/endpoints#endpoint-groups)
- [Muted](/treblle/endpoints#mute-an-endpoint) - for endpoints muted or not
