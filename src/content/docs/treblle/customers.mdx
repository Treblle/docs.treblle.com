...
Treblle does not automatically attach a Customer or User ID to API requests. If you want to track API usage by individual customers, you need to modify the request headers to include a Customer or User ID.

#### Example: Adding Customer ID in Express

To ensure every API request contains a Customer ID, you can create a middleware that extracts the authenticated user’s ID and adds it to the request headers before Treblle processes the request.
