...
Masking fields ensure that sensitive data is masked before being sent to Treblle. 
...
- **Customizable Masking**: You can define which fields should be masked when integrating the SDK.
   ...
   Example: To mask a field named `email`, you have to include it in your masking configuration. 
...
When masking the `Authorization` field, only the API key value is masked. 
...
This ensures you retain the authentication type and key length without exposing the actual API key.
...
You can define additional fields to be masked according to your application's specific requirements. 
